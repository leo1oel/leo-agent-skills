...
- Outline the logic before writing prose, so the order is an argument and not a pile of facts.
...
- Quantify. Give the number and the effect size, not "significantly" or "substantially".
- Let figures carry the argument, and write captions that state the takeaway, not just the axes.
- State limitations plainly. A weakness you name yourself reads as confidence.
